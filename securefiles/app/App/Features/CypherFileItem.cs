...
namespace IOApp.Features
{
    public partial class CypherFileItem : FileItem
    {
        ...
        public FileInfoBase OriginalInfo { get; } = new();
        public FileInfoBase EncryptedInfo { get; } = new();
        ...
        public static string? GenerateThumbnail(FileInfoBase info, uint maxWidth = 256, uint maxHeight = 256)
        {
            ...
            {
                var path = AppDir.PGetHashFilePath(AppDir.Type.TemporaryFolder, info.FullName, "thumb", "bmp");
        ...
        {
            if (Footer?.FooterThumbnailBuffer is null || Footer.Extra is null || _fileType is not ZFile.FileType.Image and ZFile.FileType.Video) return;
        ...
        public static async Task DecodeOne(string inputFilePath, string outputFilePath, string password, bool overwrite, bool ignoreException = false)
        {
            ...
            var outputRawTempFilePath = Path.Combine(outputDirPath, $"{Guid.NewGuid()}");
            var outputTempFilePath = Path.Combine(outputDirPath, $"{Guid.NewGuid()}");
